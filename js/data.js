...
const Face = {
    ...
    frontViewFace: function ($face, isCoverSlide) {
        ...
        // slide next하려면 duration 기다려야함
        let duration;
        if($face){
            let durationStyle = getComputedStyle($face[0]).transitionDuration;
            if (durationStyle) duration = durationStyle.match(/[0-9|\.]+/g)[0];
            setTimeout(function () {
                Face.canSlideNext = true;
            }, Number(duration) * 1000 + 100);
        }
        ...
        if (isCoverSlide) {
            ...
            Face.frontViewTrans(transform, duration);
        }

    },
    frontViewTrans: function (transform, duration) {
        // slide시 duration 적용. face에 transition-duration적용하면 됨
        if (duration && duration !== '0') {
            Face.observView.css('transition-duration', duration + 's')
        }
...
const Tree = {
    ...
    _bindEvents: function () {
        ...
        $('.face-list .toggle-visibility input').off('change').on('change', function () {
            ...
            const fnParentCheck = function ($ele) {
                let $parentLi = $ele.parent().closest('li');
                if ($parentLi.length > 0) {
                    let $parentVisiblityInput = $parentLi.find('> p .toggle-visibility input');
                    if (!$parentVisiblityInput.is(':checked')) {
                        $parentVisiblityInput.prop('checked', true);
                        Tree.changeFaceVisibility($parentLi.attr('face-id'), true);
                    }
                    fnParentCheck($parentLi);
                }
            }
            fnParentCheck($li);
